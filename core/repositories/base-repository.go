...
// - entity: the entity to insert.
// Returns an error if any occurs during the insertion.
func (repo *repository[T]) Create(entity *T) error {
...
// - id: the unique identifier of the entity to retrieve.
// Returns a pointer to the entity if found, and an error if any occurs during the retrieval.
func (repo *repository[T]) FindByID(id int64) (*T, error) {
	var entity T
...
// - conds: optional conditions to filter the entities.
// Returns a slice of entities and an error if any occurs during the retrieval.
func (repo *repository[T]) FindAll(conds ...interface{}) ([]T, error) {
	var entities []T
	...
	err := repo.db.Find(&entities, conds).Error
	if err != nil {
		return nil, err
	}
	return entities, nil
...
// - conds: optional conditions to filter the entities.
// Returns a slice of entities and an error if any occurs during the retrieval.
func (repo *repository[T]) FindPaginated(page int, pageSize int, conds ...interface{}) ([]T, error) {
	var entities []T
	check := checkConds(conds)
	if !check {
		result := repo.db.Offset((page - 1) * pageSize).Limit(pageSize).Find(&entities)
		if result.Error != nil {
			return nil, result.Error
		}
		return entities, nil
	}
	err := repo.db.Offset((page-1)*pageSize).Limit(pageSize).Find(&entities, conds).Error
	if err != nil {
		return nil, err
	}
	return entities, nil
...
// - conds: optional conditions to filter the entity.
// Returns a pointer to the entity if found, and an error if any occurs during the retrieval.
func (repo *repository[T]) FindOneBy(conds ...interface{}) (*T, error) {
	var entity T
	if err := repo.db.First(&entity, conds).Error; err != nil {
		return nil, err
	}
	return &entity, nil
}
...
// - id: the unique identifier of the entity to remove.
// Returns an error if any occurs during the deletion.
func (repo *repository[T]) Delete(id int64) error {
	var entity T
	return repo.db.Delete(&entity, id).Error
}
...
// - conds: optional conditions to filter the entities.
// Returns the count of entities and an error if any occurs during the counting.
func (repo *repository[T]) Count(conds ...interface{}) (int64, error) {
	var count int64
	var model T

	if err := repo.db.Model(&model).Where(conds).Count(&count).Error; err != nil {
		return 0, err
	}
	return count, nil
}
...
// - query: conditions to filter the entities.
// Returns a slice of entities that match the query and an error if any occurs during the retrieval.
func (repo *repository[T]) Where(query interface{}) ([]T, error) {
	var entities []T
	result := repo.db.Where(query).Find(&entities)
	if result.Error != nil {
		return nil, result.Error
	}
	return entities, nil
